...
export const load: PageLoad = async ({ url, fetch }) => {
	...
	if (q.trim().length === 0 && game.length === 0 && loader.length === 0) {
		return { home, q, game, loader, results: [], error: null };
	}
	try {
		const base = normalizeBase(home);
		const results = await searchProjects(base, { q, game, loader, limit: 20 }, fetch);
		return { home: base, q, game, loader, results, error: null };
	} catch (cause) {
		return {
			home,
			q,
